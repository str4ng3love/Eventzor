...
const PriceDisplay = ({price}:Props) => {
  ...
  return (
    <span className="flex items-center justify-center">{currency.name!== "initial" ? (price * currency.rate).toFixed(2): <SpinnerMini borderSize="border-[4px]" h="h-4" w="w-4" />}&nbsp;{currency.name!== "initial" ? currency.name.toLocaleUpperCase(): ""}</span>
  )
}
