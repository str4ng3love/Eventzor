...
const DisplayStock = ({ amount, amountSold }: Props) => {
    const stock = ((amount / (amount + amountSold)) * 100).toFixed(0).toString() + "%"
    const sold = ((amountSold / (amount + amountSold)) * 100).toFixed(0).toString() + '%'
    ...
    return (
        <span className={`group w-[100%] min-w-[5rem] flex justify-between items-center sm:mx-4  ring-1 ring-text px-1 rounded-sm relative`}>
            ...
            <span className={`block w-[${stock}] bg-link h-4`} />
            <span className={`block w-[${sold}] h-4`} />
        </span>
    )
}
