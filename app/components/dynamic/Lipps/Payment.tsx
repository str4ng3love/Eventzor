    ...
    return (<>
        <div className="flex flex-col">
            ...
            {processing ?
                ...
                </div> : <span className="mx-auto">
                    {completed ? <div className="animate-fadeIn bg-primary rounded-md p-8 flex flex-col items-center justify-center shadow-md shadow-black">
                        <span className="p-2 py-4 text-xl font-semibold ">Transaction completed. </span>
                        <span className="p-2 py-4 text-lg ">Thank You for using LIPPS</span>
                        <span className="p-1  text-base">You now will be redirected to Orders page in:</span>
                        <span className="text-xl py-4 pt-8 animate-ping">{count}</span>
                    </div>
                        :
                        <Button text="Imagine Paying!" title="Pay" size="text-xl" interactive={!completed} fn={() => { approvePayment() }} />}
                </span>}

        </div>
        <Notification show={notify.show} error={notify.error} message={notify.message} onAnimEnd={() => {
