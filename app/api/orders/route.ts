  ...
  if (req.method === "POST") {
    ...
    if (error) {
      return NextResponse.json({
        error: `One or more of requested items has an incorrect ID`,
      });
    }
    const [eventsDB, itemsDB] = await prisma.$transaction(async (tx) => {
      const eventsDB = await Promise.all(
        events.map(
          async (e) =>
            await tx.event.findUnique({
              where: { id: e.id },
              select: { id: true, tickets: true, ticketsSold: true },
            }),
        ),
      );
      const itemsDB = await Promise.all(
        items.map(
          async (i) =>
            await tx.marketItem.findUnique({
              where: { id: i.id },
              select: { id: true, amount: true, amountSold: true },
            }),
        ),
      );
    ...
    eventsSorted.map((e, index) => {
      if (eventsDBSorted[index] === null) {
        eventsViability = {
          items: [...eventsViability.items, e.item],
          overStock: eventsViability.overStock,
          notFound: true,
        };
      } else if (
        e.amount > eventsDBSorted[index]!.tickets ||
        e.amount + eventsDBSorted[index]!.ticketsSold >
          eventsDBSorted[index]!.tickets
      ) {
        eventsViability = {
          items: [...eventsViability.items, e.item],
          overStock: true,
          notFound: eventsViability.notFound,
        };
      }
    });
    itemsSorted.map((i, index) => {
      if (itemsDBSorted[index] === null) {
        itemsViability = {
          items: [...itemsViability.items, i.item],
          overStock: itemsViability.overStock,
          notFound: true,
        };
      } else if (
        i.amount > itemsDBSorted[index]!.amount ||
        i.amount + itemsDBSorted[index]!.amountSold >
          itemsDBSorted[index]!.amount
      ) {
        itemsViability = {
          items: [...itemsViability.items, i.item],
          notFound: itemsViability.notFound,
          overStock: true,
        };
      }
    });
    ...
    if (eventsViability.overStock === true) {
      return NextResponse.json({
        error: `There's not enough requested items : "${eventsViability.items.filter((i) => i.length > 0).map((i, index) => `${index === 0 ? "" : " "}` + i.replace(",", " "))}" tickets`,
      });
    }
    if (itemsViability.overStock === true) {
      return NextResponse.json({
        error: `There's not enough requested items : "${itemsViability.items.filter((i) => i.length > 0).map((i, index) => `${index === 0 ? "" : " "}` + i.replace(",", " "))}"`,
      });
    }
        ...
        await Promise.all(
          eventsSorted.map((e, index) =>
            tx.event.update({
              where: { id: e.id },
              data: {
                ticketsSold: eventsDBSorted[index]!.ticketsSold + e.amount,
              },
            }),
          ),
        );
        await Promise.all(
          itemsSorted.map((i, index) =>
            tx.marketItem.update({
              where: { id: i.id },
              data: { amountSold: itemsDBSorted[index]!.amountSold + i.amount },
            }),
          ),
        );
