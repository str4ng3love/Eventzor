...
async function handler(req: Request) {
  ...
  if (!body.password || !body.newPassword || !body.confirmPassword) {
    return NextResponse.json({ error: 'Please provide password, newPassword and confirmPassword fields' })
  }
  ...
  try {
    const newPassword = await prisma.$transaction(async (tx) => {
      const password = await tx.user.findUnique({ where: { name: session.user?.name as string }, select: { password: true } })
      const isMatch = await compare(body.password, password?.password as string)
      console.log(isMatch)
      if (!isMatch) {
        return NextResponse.json({ error: 'Wrong password' })
      }
      const hashNewPass = await hashPass(body.newPassword)
      const newPassword = await tx.user.update({ where: { name: session.user?.name as string }, data: { password: hashNewPass }})
      return newPassword
    })
